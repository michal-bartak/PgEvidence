...
<div class="stepper" class:disabled>
  ...
  <div class="btns">
    <button type="button" tabindex="-1" aria-label="Increase" {disabled} on:click={() => bump(1)}>
      <svg viewBox="0 0 12 8" width="11" height="8"><path d="M1 6l5-4 5 4" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </button>
    <button type="button" tabindex="-1" aria-label="Decrease" {disabled} on:click={() => bump(-1)}>
      <svg viewBox="0 0 12 8" width="11" height="8"><path d="M1 2l5 4 5-4" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </button>
  </div>
</div>
...
<style>
  ...
  .stepper input { width: 100%; padding-right: 30px; }
  .btns {
  ...
  .btns button {
    flex: 1; padding: 0; margin: 0; border: none; border-radius: 0; height: auto;
    background: var(--bg-3); color: var(--muted);
    display: flex; align-items: center; justify-content: center; cursor: pointer;
  }
  .btns button:not(:disabled):hover { background: var(--btn-hover); color: var(--text); }
  .btns button + button { border-top: 1px solid var(--border-strong); }
